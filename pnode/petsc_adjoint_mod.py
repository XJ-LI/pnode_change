...
class IJacShell:
    ...
    def mult(self, A, X, Y):
        ...
        if self.ode_.use_dlpack:
            ...
            if self.ode_.M != None:
                y[:] = np.matmul(self.ode_.M.detach().cpu().numpy(), self.ode_.shift*X.array_r) - jvp_u[0].cpu().numpy().flatten()
            else:
                y[:] = self.ode_.shift*X.array_r - jvp_u[0].detach().cpu().numpy().flatten()
        # _mat_shift_and_scale(self, X, Y)
    ...
    def multTranspose(self, A, X, Y):
        ...
        if self.ode_.use_dlpack:
            ...
            if self.ode_.M != None:
                y[:] = np.matmul(self.ode_.M.cpu().detach().numpy(), self.ode_.shift*X.array_r) - vjp_u.cpu().numpy().flatten()
            else:
...
class ODEPetsc(object):
    ...
    def evalIFunction(self, ts, t, U, Udot, F):
        if self.use_dlpack:
            ...
            dudt = self.func(t, u_tensor).cpu().detach().numpy()
            if self.M!=None:
                f[:] = np.matmul(self.M.cpu().detach().numpy(),Udot.array_r) - dudt.flatten()
            else: 
    ...
    def setupTS(self, u_tensor, func, step_size=0.01, enable_adjoint=True, implicit_form=False, use_dlpack=True, method='euler', M = None):
        """
        ...
        Args:
            ...
            M: mass matrix, only provide for DAE problems, should be tensor
        """
        ...
        if self.M != M:
            self.M = M
        if self.M!=None and use_dlpack==True:
            print('dlpack not supported for DAE problems, existing...')
            sys.exit()
        ...
        if tensor_size != self.tensor_size or tensor_dtype != self.tensor_dtype or device != self.device:
            ...
            if implicit_form:
                ...
                if M!=None:
                    print('explicit form does not support non identity mass matrix and will be ignored')
